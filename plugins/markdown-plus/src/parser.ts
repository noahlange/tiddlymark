...
function transformNodes(nodes): WidgetNode[] {
  function transformNode(node): WidgetNode {
    ...
    if (typeof node === 'string' && node.replace(/[\r\n]+/gim, '') !== '') {
...
((): void => {
  ...
  exports['text/x-markdown'] = function markdown(type: string, text: string) {
    const tree = parser(text, { inline: false });
    const output = render(tree);
    const nodes = transformNodes(output);
    this.tree = nodes;
  };
})();
